package com.delmy.caculadora.views
...
import com.delmy.caculadora.components.Alert
import com.delmy.caculadora.components.MainButton
import com.delmy.caculadora.components.MainTextField
import com.delmy.caculadora.components.ShowInfoCards
import com.delmy.caculadora.components.SpaceH
...
fun ContentHomeView(paddingValues: PaddingValues){
    var montoPrestamo by remember { mutableStateOf (value =  "")}
    var cantCuotas by remember {mutableStateOf (value = "") }
    var tasa by remember {mutableStateOf (value =  "")}
    ...
    Column( modifier= Modifier
        ...
        if (showAlert){
            Alert(
                 title = "Alerta" ,
                 message = "por fabor ingrese los datos ",
                confirmText="Aceptar",
                onConfirmClick = {
                showAlert = false }
                ){}
...
fun calcularCuota(monto: Double, cuotas: Int, tasa: Double): Double {
    val tasaInteresesMensual = tasa /12/100
    val cuota = monto * tasaInteresesMensual* Math.pow(1*tasaInteresesMensual,cuotas.toDouble())/(Math.pow(1+tasaInteresesMensual,cuotas.toDouble())-1)
